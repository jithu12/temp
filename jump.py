...
from dataviz_core.adapters.worflow_executor import WorkflowExecutor
...
class WorkspaceService(SessionManagerMixin):
    ...
    def _update_workspace_with(self, workspace: Workspace, **changes) -> dict:
        """Persist *changes* to the workspace row. Returns the raw update dict."""
        return update_resource_with(
            ctx_manager=self.autocommit(),
            repository=self.repositories.workspace,
            resource=workspace,
            logger=self.logger,
            **changes,
        )

    def _update_workspace_with_and_return(self, workspace: Workspace, **changes) -> Workspace:
        """Persist *changes* then return a fresh copy from the DB."""
        self._update_workspace_with(workspace, **changes)
        return self.repositories.workspace.get_by_id(workspace.id)
