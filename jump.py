...
from dataviz_core.adapters.celery_workflow_executor import celery_workflow_executor_from_config
...
from dataviz_core.adapters.myvault_client import myvault_client_from_config
...
from dataviz_core.repositories.sqlalchemy import get_default_session_provider
from dataviz_core.services.inventory import InventoryService
from dataviz_core.services.kube import KubeService
from dataviz_core.services.workspace import WorkspaceService
...
class _AccountLifecycleMonitoringAdapter:
    """Fallback monitoring adapter used by lifecycle handlers."""

    def __init__(self, logger) -> None:
        self._logger = logger

    def send_notification(self, error_level: str, email_subject: str, email_body: str) -> None:
        self._logger.error(
            "Lifecycle notification [%s] %s | %s",
            error_level,
            email_subject,
            email_body,
        )
...
class _AccountLifecycleGarbageCollectorAdapter:
    """Fallback garbage collector adapter used by lifecycle handlers."""

    def __init__(self, logger) -> None:
        self._logger = logger

    def set_deletion_date_for_account(self, account, grace_period_days: int) -> None:
        self._logger.info(
            "Lifecycle grace tracking set for account %s with %s day(s)",
            getattr(account, "owner_account_id", "unknown"),
            grace_period_days,
        )

    def cancel_deletion_date_for_account(self, account) -> None:
        self._logger.info(
            "Lifecycle grace tracking cancelled for account %s",
            getattr(account, "owner_account_id", "unknown"),
        )


class DatavizCore:
    """Façade over the DatavizCore module.

    Instantiation order matters: adapters first, then services, then
    cross-service wiring (circular-dependency injection last).
    """
    ...
    def __init__(
        ...
        # Sends tasks to Dataviz-Async via the queue broker.
        # FIX: was instantiated twice (here and again inside the adapters block).
        ...
        self.docker_image_client = get_default_docker_image_client(
            self.config, self.kube_rp_client
        )
        ...
        self.logger.info("Setting up Settings service...")
        ...
        self.dataplane = DataplaneService(
        ...
        self.logger.info("Setting up EventBus adapter and service...")
        ...
        # ------------------------------------------------------------------ #
        # Dataplane cluster bootstrap                                          #
        # ------------------------------------------------------------------ #
        self.logger.info("Initializing Dataviz Workspaces backend cluster...")
        # FIXME: DataplanesService should own cluster identity internally rather
        #        than requiring the caller to inject it here and into WorkspacesService.
        ...
        self.logger.info("Setting up Restore service...")
        ...
        # FIXME: dataplane_cluster_id should be resolved inside DataplanesService,
        ...
        # ------------------------------------------------------------------ #
        # Cross-service wiring (circular-dependency injection)                #
        # ------------------------------------------------------------------ #

        # AccountService needs WorkspacesService to act on lifecycle events, but
        # WorkspacesService already holds a reference to AccountService, so we
        # must inject after both are fully constructed.
        self.account.set_workspace_service(self.workspace)
        self.account.set_monitoring_service(_AccountLifecycleMonitoringAdapter(self.logger))
        self.account.set_garbage_collector_service(
            _AccountLifecycleGarbageCollectorAdapter(self.logger)
        )

        # Workflow executor is injected post-construction for the same reason:
        # AccountService constructor runs before the executor is wired into
        # services that AccountService may trigger.
        # Fix #15: use the dedicated setter instead of direct attribute assignment
        self.account.set_workflow_executor(self.workflow_executor)

        # AccountService doubles as the lifecycle event consumer: it already
        # understands lifecycle event types and owns the relevant DB state.
        # Exposed under a descriptive alias so app.py callers are explicit
        # about intent without depending on the AccountService type directly.
        self.account_lifecycle_consumer = self.account

        # ------------------------------------------------------------------ #
        # Account Reconciliation Service                                       #
        # ------------------------------------------------------------------ #
        # Initialized with accounts_api_client=None so startup is never
        # blocked when ACCOUNTS_TEAM_API_URL is not configured.
        # app.py checks _accounts_api_client before calling .run() and skips
        # gracefully if it is None.
        self.logger.info("Setting up Account Reconciliation service...")
