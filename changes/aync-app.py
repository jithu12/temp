...
def _extract_account_id(event, event_data: dict) -> Optional[str]:
    """
    Extract the ACCOUNT ID (not the resource ID) from a CloudEvent.

    Real event structure (from Accounts Team):
      "subject"       : <resource UUID>   ← this is the RESOURCE being acted on, NOT the account
      "resourceowner" : <account UUID>    ← this is the ACCOUNT ID we need

    Priority order:
      1. event.extensions["resourceowner"] – CloudEvent vendor extension, always the account UUID
      2. event.headers["resourceowner"]    – same field in some broker serialisations
      3. data.resource.ownerId (SRN)       – "srn:sgcp:account…:account:<uuid>" — parse last segment
      4. data.resource.owner_account_id    – plain UUID in some older payloads
      5. event_data["account_id"]          – legacy flat-payload key

    NOTE: "subject" is intentionally NOT used — it holds the resource UUID, not the account UUID.
    """
    # 1. CloudEvent extension: resourceowner  (primary source in real events)
    ...
    # 2. Some AMQP serialisations flatten extensions into headers
    if hasattr(event, "headers") and isinstance(event.headers, dict):
        val = event.headers.get("resourceowner") or event.headers.get("resourceOwner")
        if val:
            return str(val)

    # 3. data.resource.ownerId  – SRN format: "srn:sgcp:…:account:<uuid>"
    resource = event_data.get("resource", {}) if isinstance(event_data, dict) else {}
    if isinstance(resource, dict):
        owner_id = resource.get("ownerId", "")
        if owner_id:
            # Parse last segment of the SRN:  "srn:…:account:<uuid>"
            parts = str(owner_id).split(":")
            if parts:
                return parts[-1]

    # 4. Plain UUID in legacy payloads
    if isinstance(resource, dict):
        val = resource.get("owner_account_id")
        if val:
            return str(val)

    # 5. Legacy flat-payload key
    if isinstance(event_data, dict):
        val = event_data.get("account_id")
        if val:
            return str(val)
